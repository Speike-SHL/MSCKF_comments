...
namespace msckf_vio
{
    ...
    void MsckfVio::isTouchdownCallback(const dog_msg::isTouchdown::ConstPtr &msg)
    {
        ...
        if (!isTouchdown_buffer_bak.empty() && msg->header.stamp.toSec() <= isTouchdown_buffer_bak.back().header.stamp.toSec())
            ROS_ERROR_STREAM("触地数据时间辍混乱！last: " << isTouchdown_buffer_bak.back().header.stamp.toSec() << " now: " << msg->header.stamp.toSec());
        isTouchdown_buffer.push_back(*msg);
        isTouchdown_buffer_bak.push_back(*msg);
        if (isTouchdown_buffer_bak.size() > 100)
            isTouchdown_buffer_bak.erase(isTouchdown_buffer_bak.begin(), isTouchdown_buffer_bak.begin() + 90);
    ...
    void MsckfVio::wheelMotor_fbCallback(const dog_msg::wheel_motor_fb::ConstPtr &msg)
    {
        ...
        if (!wheelMotor_fb_buffer_bak.empty() && msg->header.stamp.toSec() <= wheelMotor_fb_buffer_bak.back().header.stamp.toSec())
            ROS_ERROR_STREAM("轮子反馈数据时间辍混乱！last: " << wheelMotor_fb_buffer_bak.back().header.stamp.toSec() << " now: " << msg->header.stamp.toSec());
        wheelMotor_fb_buffer.push_back(*msg);
        wheelMotor_fb_buffer_bak.push_back(*msg);
        if (wheelMotor_fb_buffer_bak.size() > 100)
            wheelMotor_fb_buffer_bak.erase(wheelMotor_fb_buffer_bak.begin(), wheelMotor_fb_buffer_bak.begin() + 90);
    ...
    void MsckfVio::qNow_dqNow_TNowCallback(const dog_msg::qNow_dqNow_TNow::ConstPtr &msg)
    {
        ...
        int seq = msg->header.seq;
        double cur_time = msg->header.stamp.toSec();
        ROS_INFO_STREAM_PURPLE("qNow_dqNow_TNowCallback header: seq: " << seq << " time: " << cur_time);
        // BUG？ 清理, 不然内存一直增大
        ...
        if (merge_leg == true)
        {
            ...
            // INFO 测试时间戳的输出
            /* GREENCOUT("[before] isTouchdown_buffer size: " << isTouchdown_buffer.size());
            for(auto &isTouchdown_msg : isTouchdown_buffer)
                cout << "< seq: " << isTouchdown_msg.header.seq << " time: " << isTouchdown_msg.header.stamp.toSec() << " > ";
            cout << endl;
            GREENCOUT("[before] wheelMotor_fb_buffer size: " << wheelMotor_fb_buffer.size());
            for(auto &wheelMotor_fb_msg : wheelMotor_fb_buffer)
                cout << "< seq: " << wheelMotor_fb_msg.header.seq << " time: " << wheelMotor_fb_msg.header.stamp.toSec() << " > ";
            cout << endl; */
            ...
            // INFO 测试时间戳的输出
            /* YELLOWCOUT("[after] isTouchdown_buffer_tmp size: " << isTouchdown_buffer_tmp.size());
            for(auto &isTouchdown_msg : isTouchdown_buffer_tmp)
                cout << "< seq: " << isTouchdown_msg.header.seq << " time: " << isTouchdown_msg.header.stamp.toSec() << " > ";
            cout << endl;
            YELLOWCOUT("[after] isTouchdown_buffer size: " << isTouchdown_buffer.size());
            for(auto &isTouchdown_msg : isTouchdown_buffer)
                cout << "< seq: " << isTouchdown_msg.header.seq << " time: " << isTouchdown_msg.header.stamp.toSec() << " > ";
            cout << endl;
            YELLOWCOUT("[after] wheelMotor_fb_buffer_tmp size: " << wheelMotor_fb_buffer_tmp.size());
            for(auto &wheelMotor_fb_msg : wheelMotor_fb_buffer_tmp)
                cout << "< seq: " << wheelMotor_fb_msg.header.seq << " time: " << wheelMotor_fb_msg.header.stamp.toSec() << " > ";
            cout << endl;
            YELLOWCOUT("[after] wheelMotor_fb_buffer size: " << wheelMotor_fb_buffer.size());
            for(auto &wheelMotor_fb_msg : wheelMotor_fb_buffer)
                cout << "< seq: " << wheelMotor_fb_msg.header.seq << " time: " << wheelMotor_fb_msg.header.stamp.toSec() << " > ";
            cout << endl; */
            ...
            for (int id = 0; id < 4; ++id)
            {
                ...
                else if (found && true_contact) // 3. 在状态量中, 真的触地
                {
                    ...
                    // 使用轮速数据对状态量中的该腿进行位置补偿, 这里肯定是触地的，不用考虑isTouchdown_buffer

                    // INFO 测试时间戳的输出
                    /* cout << "wheelMotor_fb_buffer_tmp size: " << wheelMotor_fb_buffer_tmp.size() << " leg " << id << " time: " << state_server.leg_state.legs[leg_id].time << endl;
                    for (auto &wheelMotor_fb_msg : wheelMotor_fb_buffer_tmp)
                        cout << "< seq: " << wheelMotor_fb_msg.header.seq << " time: " << wheelMotor_fb_msg.header.stamp.toSec() << " > ";
                    cout << endl; */
                    ...
                    if (wheelMotor_fb_buffer_tmp.size() > 1)
                    {
                        ...
                        for (int i = 0; i < wheelMotor_fb_buffer_tmp.size() - 1; ++i)
                        {
                            ...
                            // INFO 测试时间戳的输出
                            // cout << "i: " << i << " wheelMotor_fb_buffer_tmp[i].header.stamp.toSec(): " << wheelMotor_fb_buffer_tmp[i].header.stamp.toSec()
                            //     << "leg " << id << " time: " << state_server.leg_state.legs[leg_id].time << " dt:" << dt << endl;
                            // FIXME 由于腿部数据的时间戳没有对齐，所以这里有误差，会存在dt < 0的情况，即wheelMotor的buffer中会出现小于上次qNow msg时间的数据，目前没好的方法，只能暂时跳过这些数据
                            if (dt < 0)
                            {
                            ...
                            // 使用真实值
                            if (id == 0)
                                v_w = webotsRealState.R_NWU2NUE * webotsRealState.FLlun_velocity_in_world;
                            else if (id == 1)
                                v_w = webotsRealState.R_NWU2NUE * webotsRealState.RLlun_velocity_in_world;
                            else if (id == 2)
                                v_w = webotsRealState.R_NWU2NUE * webotsRealState.RRlun_velocity_in_world;
                            else if (id == 3)
                                v_w = webotsRealState.R_NWU2NUE * webotsRealState.FRlun_velocity_in_world;
                            d_GI += v_w * dt;
                            ...
                            // cout << "leg " << id << " time: " << state_server.leg_state.legs[leg_id].time << endl;
                        }
                    }
                    if (!wheelMotor_fb_buffer_tmp.empty())
                    ...
                    // cout << "leg " << id << " time: " << state_server.leg_state.legs[leg_id].time << " msg time: " << msg->header.stamp.toSec() << " dt:" << dt << endl;
                    if (dt < 0)
                    {
                        ROS_DEBUG_STREAM("msg time: " << msg->header.stamp.toSec());
                        ROS_DEBUG_STREAM("state_server.leg_state.legs[leg_id].time: " << state_server.leg_state.legs[leg_id].time);
                        ROS_WARN("轮速预估时间戳错误 - 2");
